...
| __Receive Shadows__ | Check this box to enable your GameObject to have shadows cast upon it by other objects. If you uncheck this box, the GameObject will not have shadows on it. |
...
Under __Source__, you can control where to sample a smoothness map from. By default, both the metallic and specular workflow uses the Alpha channel of its map for the source. You can also set it to the Base Map Alpha channel. |
...
| __Emission__                | Makes the surface look like it emits lights. When enabled, the  __Emission Map__ and __Emission Color__ settings appear.<br/>To assign an __Emission Map__, click the object picture next to it. This opens the Asset Browser, where you can select from the textures in your Project.
...
If you do not enable __Emission__, LWRP sets the emission to black and does not calculate emission. |
